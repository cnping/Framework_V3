...
void * DBCtrl::getElementList(String8 columnName, int columnIdx, unsigned int rowNumber, int filetype)
{
	int errorCode;
	void *retAddr = NULL;
	char buf[20];
	...
	if (filetype == 4){
	    str.append("WHERE file like '%.jpg'");

	}else if(filetype == 1){
        str.append("WHERE file like '%.mp4'AND file like '%A%' AND \
		file not like '%B%' AND file not like '%SOS%' ");

	}else if(filetype == 2){
        str.append("WHERE file like '%.mp4'AND file like '%B%' AND file not \
		like '%A%' AND file not like '%SOS%' ");

	}else if(filetype == 3){
     //   str.append("WHERE file like '%.mp4'AND file like '%SOS%' \
     //		AND file not like '%B%' AND file not like '%A%' ");
        str.append("WHERE file like '%.mp4'AND file like '%SOS%' ");
